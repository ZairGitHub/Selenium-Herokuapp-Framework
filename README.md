...
The framework's test suite uniquely supports the execution of standard TDD tests written in NUnit and BDD tests written in SpecFlow. These have been created to aid a learning exercise which focuses on understanding the implementation differences and the advantages and disadvantages each approach bears against one another. My findings are as follows: 
